...
| `/#/quiz` | Quiz | `QuizView.tsx` → `QuizEngine.tsx` → `QuizResults.tsx` | Quiz complet avec 87 questions (annales + originales), score temps réel, écran de résultats détaillé avec jauge circulaire et feedback par question |
| `/#/calculator` | Calculateur | `PriceCalculator.tsx` | Calculateur interactif de cascade des prix export (EXW → DDP), calcul automatique de l'assurance |
...
├── core/
│   └── calculatorLogic.ts           # Logique mathématique pure
├── data/
│   ├── incoterms2020Data.json       # Base de données des 11 Incoterms
│   ├── quizQuestions.json           # 87 questions fusionnées (source de vérité quiz)
...
### 3.3 Formule d'assurance — Règle des 110 %

La formule **universelle** du calcul de la prime d'assurance transport en BTS CI :

```
Prime = Valeur CIF (ou CIP) × 1,10 × taux
```

- Le coefficient **1,10** (majoration de 10 %) couvre le bénéfice escompté de l'acheteur.
- **Formule algébrique exacte** (calcul circulaire) : `CIF = CFR / (1 - taux × 1,10)`
- **Approximation** (admise en BTS) : `Prime ≈ CFR × 1,10 × taux`

### 3.4 Assurance obligatoire CIP vs CIF (PIÈGE EXAMEN 2020)
...
### 3.5 Valeur en douane UE (Art. 70 CDU)
...
### 3.7 THC dans la cascade multimodale
...
### 4.1 Fichier `quizQuestions.json` — 87 questions
...
| `"Entraînement IncoMaster"` | Question originale créée pour l'entraînement | 20 |
...
- [ ] **Code-splitting** : Lazy loading des vues avec `React.lazy()` pour réduire le bundle (actuellement 657 kB)
- [ ] **Tests unitaires** : Couvrir `calculatorLogic.ts` et les formules d'assurance
